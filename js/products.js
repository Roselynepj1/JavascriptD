...
const renderCheckOutProducts = async()=>{
  ...
  try{
    ...
    const checkOutProductsIds = getCheckOutItems()
    
    if(!checkOutProductsIds.length) return checkOutContainer.appendChild(showNoCheckOutProducts())

    //iterate all checkout products and filter them out from the list of all products
    
    checkOutProductsIds.forEach((checkoutProductId)=>{
      const found = products.find(product => product.id === checkoutProductId)
       
      if(found){
        checkOutContainer.appendChild(createCheckoutElement(found))
      }
    })
